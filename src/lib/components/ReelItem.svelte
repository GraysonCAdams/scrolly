<script lang="ts">
	...
	import { setupDesktopGestures, setupMobileGestures, setupReelKeyboard } from '$lib/reelInteractions';
	...
	const {
		clip, currentUserId, active, index, autoScroll,
		canEditCaption = false, seenByOthers = false,
		onwatched, onfavorited, onreaction, onretry, onended, oncaptionedit, ondelete
	}: {
		clip: FeedClip;
	...
	const unsubMute = globalMuted.subscribe((v) => { muted = v; });
	const unsubSpeed = globalPlaybackSpeed.subscribe((v) => { speed = v; });
	...
	onMount(() => { isDesktop = isPointerFine(); });
	onDestroy(() => { unsubMute(); unsubSpeed(); sendWatchPercent(clip.id, maxPercent); });

	// Mark watched after 3s visible
	$effect(() => {
		if (!active || clip.watched || hasMarkedWatched) return;
		const timer = setTimeout(() => { hasMarkedWatched = true; onwatched(clip.id); }, 3000);
	...
	$effect(() => {
		...
		return trackVideoTime(
			videoEl,
			(t, d, p) => { currentTime = t; duration = d; paused = p; },
			(d) => { duration = d; },
			(pct) => { if (pct > maxPercent) maxPercent = pct; }
		);
	});
	...
	function handleCycleSpeed() { cycleSpeed(); showSpeedChange(); }
	function seek(seconds: number) { if (videoEl) videoEl.currentTime = Math.max(0, Math.min(videoEl.currentTime + seconds, duration)); }
	function seekTo(time: number) { if (videoEl) videoEl.currentTime = Math.max(0, Math.min(time, duration)); }

	function fireHeartReaction(cx: number, cy: number) {
		showerEmoji = '❤️'; showerX = cx; showerY = cy; showShower = true;
		if (!clip.reactions['❤️']?.reacted) onreaction(clip.id, '❤️');
	}

	function openPicker(cx: number, cy: number, drag: boolean) {
		pickerX = cx; pickerY = cy; pickerDragMode = drag; showPicker = true;
	}

	// Gesture handler
	$effect(() => {
		if (!itemEl) return;
		const callbacks = { togglePlayPause, fireHeartReaction, openPicker, isMusic: clip.contentType === 'music' };
		return isDesktop ? setupDesktopGestures(itemEl, callbacks) : setupMobileGestures(itemEl, callbacks);
	});
	...
	$effect(() => {
		...
		return setupReelKeyboard(
			{ toggleMute, togglePlayPause, stepSpeedUp, stepSpeedDown, showSpeedChange, seek, isMusic: clip.contentType === 'music' },
	...
	function handlePickEmoji(emoji: string) {
		showPicker = false;
		showerEmoji = emoji; showerX = pickerX; showerY = pickerY; showShower = true;
		if (!clip.reactions[emoji]?.reacted) onreaction(clip.id, emoji);
	}

	function triggerReactionPickerHold(bx: number, by: number) {
		pickerX = bx; pickerY = by; pickerDragMode = true; showPicker = true;
	}
...
<div class="reel-item" data-index={index} bind:this={itemEl}>
	...
	<div class="speed-pill" class:visible={active} class:highlight={speed !== 1} onclick={(e) => { e.stopPropagation(); handleCycleSpeed(); }}>
		{speed}x
	</div>

	<ReelIndicators {showSpeedIndicator} {speed} {showMuteIndicator} {muted} {showPlayIndicator} {paused} isMusic={clip.contentType === 'music'} />

	{#if clip.contentType !== 'music' && duration > 0}
		<ProgressBar {currentTime} {duration} {isDesktop} onseek={seekTo} />
	{/if}

	<ReelOverlay username={clip.addedByUsername} avatarPath={clip.addedByAvatar} platform={clip.platform} caption={clip.title} reactions={clip.reactions} onreaction={(emoji) => onreaction(clip.id, emoji)} {canEditCaption} {seenByOthers} clipId={clip.id} {oncaptionedit} {ondelete} />

	<ActionSidebar favorited={clip.favorited} commentCount={clip.commentCount} unreadCommentCount={localUnreadCount} originalUrl={clip.originalUrl} {muted} onfavorite={() => onfavorited(clip.id)} oncomment={() => { commentsAutoFocus = false; showComments = true; }} onreaction={handleSidebarReactionTap} onreactionhold={triggerReactionPickerHold} onmute={toggleMute} />
	...
	{#if active}
		<!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
		<div class="comment-prompt" onclick={(e) => { e.stopPropagation(); commentsAutoFocus = true; showComments = true; }}>
...
{#if showPicker}
	<ReactionPicker x={pickerX} y={pickerY} dragMode={pickerDragMode} onpick={handlePickEmoji} ondismiss={() => (showPicker = false)} />
{/if}
{#if showShower}
	<EmojiShower emoji={showerEmoji} x={showerX} y={showerY} oncomplete={() => (showShower = false)} />
{/if}
{#if showComments}
	<CommentsSheet clipId={clip.id} {currentUserId} autoFocus={commentsAutoFocus} ondismiss={() => { showComments = false; localUnreadCount = 0; }} />
{/if}
{#if showViewers}
...
<style>
	.reel-item { height: 100%; width: 100%; position: relative; overflow: hidden; background: var(--bg-primary); }
	.bottom-gradient {
		position: absolute;
	...
	.view-badge-wrapper { position: absolute; top: max(60px, calc(env(safe-area-inset-top) + 52px)); left: var(--space-lg); z-index: 6; }
	.speed-pill {
		position: absolute;
	...
	.speed-pill.visible { opacity: 1; pointer-events: auto; }
	.speed-pill.highlight { background: rgba(255, 255, 255, 0.2); color: #fff; }
	.speed-pill:active { transform: scale(0.93); }
	.comment-prompt {
		position: absolute;
	...
	.comment-prompt:active { background: rgba(255, 255, 255, 0.2); }
	.comment-prompt svg { width: 18px; height: 18px; flex-shrink: 0; color: rgba(255, 255, 255, 0.5); }
	.comment-prompt span { font-size: 0.875rem; color: rgba(255, 255, 255, 0.5); }
	...
	.music-disc.spinning { animation: spin-disc 4s linear infinite; }
